...
describe Res::Reporters::TestRail do
  ...
  before(:all) do
    if File.exists?("./examples/cucumber/.test_rail.yaml")
      @exists = true
    else
      @exists = false
    end
  ...
  describe "Submitting Junit results in testrail" do
    describe "Create a new test run and submit results" do
      ...
      it "Skipping tests", :if => !@exists do
      ...
      it "Submit the test results", :if => @exists do
  ...
  describe "Submitting Cucumber results in testrail" do
    describe "Create a new test run and submit results" do
      ...
      it "Submit the test results", :if => @exists do
        ...
        @output = `RUBYLIB=lib bundle exec ruby ./bin/res.rb --res examples/cucumber/cucumber.res --submit testrail --config-file ./examples/cucumber/.test_rail.yaml`
